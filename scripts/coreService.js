...
serverDataLoadOtherPlayers() {
  try {
    console.log('Початок виконання serverDataLoadOtherPlayers');
    this.loadFromServerOtherPlayers().then(success => {
      if (success) {
        console.log('Дані успішно отримані, оновлюємо стан');
        this.eventsCore.emit('statsUpdated');
        this.saveState();
      } else {
        console.log('Дані не отримані, стан не оновлено');
      }
    });
  } catch (error) {
